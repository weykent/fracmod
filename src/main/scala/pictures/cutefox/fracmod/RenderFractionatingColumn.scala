...
class RenderFractionatingColumn extends FastTESR[TileFractionatingColumn] {
  ...
  override def renderTileEntityFast(tile: TileFractionatingColumn, x: Double, y: Double, z: Double,
    ...
    val lightValue = tile.fluids
      .map { f => f.getFluid.getLuminosity(f) }
      .max
    ...
    for (f <- tile.fluidsToRender) {
      ...
      FluidRenderer.renderFluid(
        ...
        new Vec3d(MIN.x - xzJitter, MIN.y + Y_RANGE * f.fractionBottom - (if (f.showBottom) 0 else EPSILON), MIN.z - xzJitter),
        new Vec3d(MAX.x + xzJitter, MIN.y + Y_RANGE * f.fractionTop + (if (f.showTop) 0 else EPSILON), MAX.z + xzJitter),
        buffer, sideRender)
    }
